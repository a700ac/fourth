...
public class Main {
    ...
    String[] rim= {"O", "I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X", "XI", "XII", "XIII", "XIV",
        ...
        for (int i=0;i<rim.length; i++) {
            if  (arr[0].equals(rim[i])) { operand1 = i; chk1=true; }
        }
        for (int i=0;i<rim.length; i++) {
            if (arr[1].equals(rim[i])) { operand2 = i; chk2=true;}
        }
        //Проверки на правильность ввода;
        if ((chk1==true)&&(chk2==false)||((chk1==false)&&(chk2==true))) {throw new Exception("Используются одновременно" +
                " разные системы счисления"); }


        if (chk1==false) {operand1 = Integer.parseInt(arr[0]);}
        if (chk2==false) {operand2 = Integer.parseInt(arr[1]);}

        if ((operand1>10)||(operand2>10)||(operand1==0)||(operand2==0)) throw new Exception("Вводимые числа могут быть" +
                " от 1 до 10");
        ...
        if ((chk1==true)&&(chk2==true)&&(res<1))  throw new Exception("В римской системе нет отрицательных чисел и нуля");
            if ((chk1==true)&&(chk2==true))  {System.out.println(rim[res]);
